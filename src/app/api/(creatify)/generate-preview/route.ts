  ...
  try {
    ...
    if (!body.link || !body.override_script) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: link and override_script are required",
        },
        { status: 400 }
      );
    }
    ...
    if (!createLinkResponse.ok) {
      ...
      return NextResponse.json(
        {
          error: "Failed to create link in Creatify system",
          details: errorData,
        },
        { status: createLinkResponse.status }
      );
    ...
    const previewPayload = {
      ...
      aspect_ratio: body.aspect_ratio || "9x16", // Default to 16:9 as specified
      ...
      visual_style: "DynamicGreenScreenEffect", // Default visual style
      script_style: "DIY", // DIY script style
      override_avatar: "f6277bad-2e12-4299-b992-1883410e0d4f", // Hardcoded avatar as requested
    ...
    if (!previewResponse.ok) {
      ...
      return NextResponse.json(
        {
          error: "Failed to generate link to video preview",
          details: errorData,
        },
        { status: previewResponse.status }
      );
    ...
    const previewData: CreatifyLinkPreviewResponse =
